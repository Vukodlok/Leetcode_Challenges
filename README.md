...
- Best Time to Buy and Sell Stock IV: Find max profit from a specified number of trades.
...
- House Robber: Given a list of integers, find the max sum without using adjacent values.
...
- Longest Palindromic Substring: Find smallest palindromic substring.
...
- Max Points on a Line: Find max number of points on a line.
...
- Minimum Path Sum: Find the cheapest path for the top left to bottom right path in a grid.
...
- Word Break: Determine if a string can be made from strings in a dictionary.
